...
// Native bcrypt — bcryptjs (pure JS) blocks the entire Node event loop for the
// full ~100ms of every hash/compare call (confirmed by timing a concurrent
// setInterval tick across the call). Under any concurrent traffic that stalls
// every other in-flight request for that whole window, which is why the app
// can look like "the database is down" right around login. Native bcrypt runs
// the computation on a libuv worker thread, so it doesn't block anything.
...
export function setupAuth(app: Express) {
  ...
  // 10 years — practically "never expires". Store/staff logins are expected to
  // stay signed in indefinitely (this used to be a 7-day default with an
  // unreachable "keep me signed in" opt-in — the login page never actually
  // rendered that checkbox, so every real login silently got logged out after
  // a week of inactivity). `rolling: true` below still refreshes this on every
  // request, so an active session's expiry keeps sliding forward regardless.
   ...
   * ──────────────────────────────
   * Consumes a single-use magic link generated by a support agent
   * (POST /api/support/accounts/:id/magic-link) and logs the account owner
   * in directly, without a password. The token is looked up in
   * support_magic_links (created lazily by routes/support.ts), checked for
   * expiry and single use, then marked used before the session is created.
   */
  app.get("/api/auth/magic", async (req, res) => {
    const token = typeof req.query.token === "string" ? req.query.token : "";
    if (!token) return res.redirect("/auth?error=invalid_magic_link");

    try {
      const { rows } = await pool.query<{ id: number; user_id: string }>(
        `SELECT id, user_id FROM support_magic_links
         WHERE token = $1 AND expires_at > NOW() AND used_at IS NULL
         LIMIT 1`,
        [token]
      );
      if (!rows.length) {
        return res.redirect("/auth?error=invalid_magic_link");
      }
      const magicLink = rows[0]!;

      const [user] = await db.select().from(users).where(eq(users.id, magicLink.user_id));
      if (!user) {
        return res.redirect("/auth?error=invalid_magic_link");
      }

      // Mark used immediately so the link cannot be replayed, even if the
      // session below somehow fails to establish.
      await pool.query(`UPDATE support_magic_links SET used_at = NOW() WHERE id = $1`, [magicLink.id]);

      req.session.regenerate((regenErr) => {
        if (regenErr) {
          console.error("[magic-link] Session regenerate ERROR:", regenErr);
          return res.redirect("/auth?error=invalid_magic_link");
        }
        (req.session as any).userId = user.id;
        req.session.save((err) => {
          ...
          logAuthEvent(user.id, "magic_link", req);
          return res.redirect("/auth?magic=1");
        });
      });
    } catch (error) {

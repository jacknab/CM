...
import { Calendar, MessageSquare, Phone, DollarSign, Activity } from "lucide-react";
import type { ActivityEvent } from "@/lib/support-api";
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="overflow-y-auto scrollbar-thin max-h-[240px]">
        {isLoading ? (
          <div className="p-6 flex justify-center"><div className="w-5 h-5 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" /></div>
        ) : events.length === 0 ? (
          <div className="p-6 text-center"><Activity size={24} className="text-slate-200 mx-auto mb-2" /><p className="text-xs text-slate-400">No recent activity</p></div>
        ) : (
          <div className="divide-y divide-slate-50">
            {events.map((e, i) => (
              <div key={e.id ?? i} className="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition">
                ...
                <span className="text-[10px] text-slate-400 flex-shrink-0 mt-0.5">{safeDistanceToNow(e.occurred_at, { addSuffix: true })}</span>
              </div>
            ))}
          </div>
        )}

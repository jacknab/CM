...
interface CategoryDraft {
  ...
  isAutoGenerated?: boolean;  // true if created by AI
}

interface AddonDraft extends ServiceDraft {
...
function getConfidenceColor(confidence?: number): string {
  ...
  return level === "high" ? "bg-emerald-100 text-emerald-700" :
...
export default function ServiceImportFlow() {
  ...
  const startEdit = (catIdx: number, svcIdx: number) => {
    ...
    setEditForm({ ...categories[catIdx].services[svcIdx], category: categories[catIdx].name });
  };

  const saveEdit = () => {
    if (!editingService) return;
    setCategories((prev) => {
      const next = prev.map((cat) => ({ ...cat, services: [...cat.services] }));
      const source = next[editingService.catIdx];
      if (!source) return prev;
      const [service] = source.services.splice(editingService.svcIdx, 1);
      if (!service) return prev;
      const targetName = editForm.category?.trim() || source.name;
      const target = next.find((cat) => cat.name === targetName) ?? source;
      target.services.push({ ...editForm, category: undefined });
      return next.filter((cat) => cat.services.length > 0);
    });
  ...
  const createCategory = () => {
    ...
    if (!name) return;
    const exists = categories.some((category) => category.name.trim().toLowerCase() === name.toLowerCase());
    if (exists) {
      toast({ variant: "destructive", title: "Category already exists" });
      return;
    }
    setCategories((prev) => [...prev, { name, services: [], confidence: 1, isAutoGenerated: false }]);
    setNewCategoryName("");
  ...
  const handlePublish = async () => {
    ...
    if (nonEmpty.length === 0 && addons.length === 0) {
      toast({ variant: "destructive", title: "Nothing to publish", description: "No services or add-ons were found." });
      return;
    }
    ...
    try {
      const res = await fetch("/api/service-import/publish", {
        ...
        credentials: "include",
        body: JSON.stringify({ jobId, categories: nonEmpty, addons }),
      });
  ...
  const renderReview = () => {
    const autoGeneratedCount = categories.filter(c => c.isAutoGenerated).length;
    ...
    <div className="max-w-lg mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 ml-11">
          {categories.length} categor{categories.length === 1 ? "y" : "ies"} · {totalServices} service{totalServices === 1 ? "" : "s"} · {totalAddons} add-on{totalAddons === 1 ? "" : "s"} found
          {autoGeneratedCount > 0 && <span> · {autoGeneratedCount} new categor{autoGeneratedCount === 1 ? "y" : "ies"}</span>}
        </p>
      </div>

      {/* Info box for auto-generated categories */}
      {autoGeneratedCount > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-6">
          <p className="text-sm text-amber-800">
            <strong>✨ {autoGeneratedCount} new categor{autoGeneratedCount === 1 ? "y" : "ies"} created</strong> based on your services. You can rename or remove them below.
          </p>
        </div>
      )}

      <div className="border border-dashed border-violet-300 bg-violet-50/50 rounded-2xl p-4 mb-6">
        <p className="text-sm font-semibold text-slate-800 mb-1">Need another category?</p>
        <p className="text-xs text-slate-500 mb-3">
          Create one here, then use each service&apos;s category dropdown to move services into it before publishing.
        </p>
        <div className="flex gap-2">
          <input
            ...
            placeholder="e.g. Lash Services"
            aria-label="New category name"
            className="flex-1 min-w-0 px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"
          />
          <button
            type="button"
      ...
      {categories.length === 0 ? (
        ...
        <div className="space-y-4 mb-8">
          {categories.map((cat, catIdx) => (
            <div key={catIdx} className={`border rounded-2xl overflow-hidden ${cat.isAutoGenerated ? "border-amber-200 bg-amber-50/30" : "border-slate-200"}`}>
              {/* Category header */}
              <div className={`flex items-center justify-between px-4 py-3 border-b ${cat.isAutoGenerated ? "bg-amber-50 border-amber-100" : "bg-slate-50 border-slate-200"}`}>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {cat.confidence !== undefined && (
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getConfidenceColor(cat.confidence)}`}>
                          {getConfidenceLevel(cat.confidence) === "high" ? "High Match" :
                           getConfidenceLevel(cat.confidence) === "medium" ? "Good Match" :
                           "Low Match"}
                        </span>
                      )}
                    </div>
                    <span className="text-xs text-slate-500">({cat.services.length} service{cat.services.length === 1 ? "" : "s"})</span>
                  </div>
                </div>
                <button
                    ...
                    {editingService?.catIdx === catIdx && editingService?.svcIdx === svcIdx ? (
                      ...
                      <div className="p-4 bg-violet-50 space-y-3">
                        ...
                        <div>
                          ...
                          <select
                            value={editForm.category ?? categories[editingService.catIdx]?.name ?? ""}
                            onChange={(e) => setEditForm((f) => ({ ...f, category: e.target.value }))}
                            className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"
                          >
      ...
      {addons.length > 0 && (
        <div className="border border-violet-200 rounded-2xl overflow-hidden mb-8">
          <div className="px-4 py-3 bg-violet-50 border-b border-violet-100">
            <span className="font-semibold text-slate-800">Add-ons</span>
            <span className="block text-xs text-slate-500 mt-1">
              These will be added to your Add-Ons catalog, not offered as standalone services.
            </span>
          </div>
          <div className="divide-y divide-slate-100">
            {addons.map((addon, index) => (
              <div key={`${addon.name}-${index}`} className="flex items-center gap-3 px-4 py-3">
                <Check className="w-4 h-4 text-violet-500 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-slate-700 truncate block">{addon.name}</span>
                  <span className="text-xs text-slate-400">{addon.duration} min</span>
                </div>
                <span className="text-sm font-semibold text-slate-800">{fmt(addon.price)}</span>
                <button
      ...
      <div className="sticky bottom-0 bg-white pt-4 pb-6 border-t border-slate-100 -mx-4 px-4">
        ...
        >
          {publishing ? (
            ...
            `Publish Menu · ${totalServices} service${totalServices === 1 ? "" : "s"}${totalAddons > 0 ? ` + ${totalAddons} add-on${totalAddons === 1 ? "" : "s"}` : ""}`
          )}
        </button>

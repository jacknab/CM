...
export default function TeamLoginPage() {
  const { agent, login } = useSupportAuth();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");

  if (agent) return <Navigate to="/isTeam/accounts" replace />;
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                ...
                placeholder="agent@certxa.com" autoFocus required />
            </div>
            <div>
            ...
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2.5 text-red-400 text-sm">{error}</div>
            )}

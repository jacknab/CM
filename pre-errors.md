...
4. **Quick Ticket's icon is blank.** `nail.quickticket` uses `icon: "Zap"` (`lib/pos/configs/nailSalon.ts`) but `Zap` isn't in `lib/pos/iconMap.ts`, so `resolvePosIcon` returns nothing and the tile shows a "•". (Add `Zap` to the import and `POS_ICONS` in iconMap.ts.) Noticed 2026-09-21 while making the checkout function grid uniform.

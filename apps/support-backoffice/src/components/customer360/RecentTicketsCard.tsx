...
export default function RecentTicketsCard({ accountId, showCreate }: { accountId: number; showCreate?: boolean }) {
  ...
  const { data: tickets = [], isLoading } = useQuery<TicketType[]>({
    ...
    queryFn: () => api.accounts.tickets(accountId),
    staleTime: 30_000,
  });

  const createTicket = useMutation({
    mutationFn: () => api.accounts.createTicket(accountId, form),
    onSuccess: () => {
      setCreating(false);
      setForm({ subject: "", description: "", priority: "normal" });
      qc.invalidateQueries({ queryKey: ["support-tickets", accountId] });
    },
  });
      ...
      {(creating || showCreate) && creating && (
        <div className="px-4 py-3 bg-violet-50 border-b border-violet-100 space-y-2">
          <input
            ...
            placeholder="Subject"
            className="w-full text-xs border border-violet-200 rounded-lg px-2.5 py-2 focus:outline-none focus:border-violet-400 bg-white"
          />
          <textarea
            value={form.description}
            onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            placeholder="Description (optional)"
            className="w-full text-xs border border-violet-200 rounded-lg px-2.5 py-2 focus:outline-none focus:border-violet-400 bg-white resize-none"
            rows={2}
          />
          <div className="flex items-center justify-between gap-2">
            <select
              ...
              className="text-xs border border-violet-200 rounded-lg px-2 py-1.5 bg-white focus:outline-none"
            >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="text-xs bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white px-3 py-1.5 rounded-lg transition font-medium"
              >
      ...
      <div className="max-h-[220px] overflow-y-auto scrollbar-thin">
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-50">
            {tickets.map(t => (
              <div key={t.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-bold uppercase ${priorityColors[t.priority] ?? "text-slate-600"}`}>
                      {t.priority}
                    </span>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    {t.ticket_number} · {formatDistanceToNow(new Date(t.created_at), { addSuffix: true })}
                  </p>
                </div>
              </div>
            ))}

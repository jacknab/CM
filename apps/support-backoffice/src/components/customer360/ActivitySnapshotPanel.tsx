...
import { Edit3, Plus, X, Tag, Users, ChevronDown } from "lucide-react";
...
function SnapshotSection({ store, owner, subscription, staffCount }: Omit<Props, "accountId">) {
  ...
  const rows: { label: string; value: string }[] = [
    ...
    { label: "Trial End Date",   value: fmt(owner.trialEndsAt) },
    { label: "Next Billing",     value: fmt(subscription?.renewalDate) },
    { label: "Payment Method",   value: subscription?.paymentLast4 ? `${subscription.paymentBrand ?? ""} ····${subscription.paymentLast4}` : "—" },
  ];

  return (
    <div className="p-3 border-b border-slate-100">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-slate-700">Customer Snapshot</p>
        <button className="flex items-center gap-1 text-[10px] text-indigo-600 hover:text-indigo-800 font-medium">
          <Edit3 size={10} />
          Edit
        </button>
      </div>
...
interface TagItem { id: number; tag: string; color: string; }
...
function TagsSection({ accountId }: { accountId: number }) {
  ...
  const addTag = useMutation({
    mutationFn: ({ tag, color }: { tag: string; color: string }) =>
      api.accounts.addTag(accountId, tag, color),
    onSuccess: () => {
      setNewTag(""); setAdding(false);
      qc.invalidateQueries({ queryKey: ["support-tags", accountId] });
    },
  });

  const removeTag = useMutation({
    mutationFn: (id: number) => api.accounts.removeTag(accountId, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["support-tags", accountId] }),
  });
      ...
      {adding && (
        <div className="mb-2 space-y-1.5">
          <input
            ...
            onKeyDown={e => { if (e.key === "Enter" && newTag.trim()) addTag.mutate({ tag: newTag.trim(), color: newColor }); }}
          ...
          <div className="flex items-center gap-1 flex-wrap">
            {COLOR_OPTIONS.map(([c, cfg]) => (
              <button
                key={c}
                onClick={() => setNewColor(c)}
                className={clsx("w-5 h-5 rounded-full border-2 transition", cfg.bg, newColor === c ? "border-slate-600 scale-110" : "border-transparent")}
                title={c}
              />
            ))}
          </div>
          <div className="flex gap-1.5">
            ...
            <button onClick={() => newTag.trim() && addTag.mutate({ tag: newTag.trim(), color: newColor })}
              disabled={!newTag.trim() || addTag.isPending}
              className="text-xs bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-3 py-1 rounded transition">
        ...
        {tags.map(t => {
          ...
          return (
            <span key={t.id} className={clsx("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border", cfg.bg, cfg.text, cfg.border)}>
              {t.tag}
              <button onClick={() => removeTag.mutate(t.id)} className="hover:opacity-70 transition ml-0.5">
                <X size={9} />
              </button>
            </span>
          );
...
interface OwnerItem { id: string; email: string; name: string; role: string; profileImageUrl?: string; }

function AccountOwnersSection({ accountId }: { accountId: number }) {
  const { data: owners = [] } = useQuery<OwnerItem[]>({
    queryKey: ["support-owners", accountId],
    queryFn: () => api.accounts.owners(accountId),
    staleTime: 60_000,
  });
    ...
    <div className="p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          ...
          <p className="text-xs font-semibold text-slate-700">Account Owners</p>
        </div>
      </div>
        ...
        {owners.map(o => {
          ...
          return (
            <div key={o.id} className="flex items-center gap-2">
              {o.profileImageUrl ? (
                <img src={o.profileImageUrl} alt={o.name} className="w-7 h-7 rounded-full object-cover flex-shrink-0" />
              ) : (
                <div className="w-7 h-7 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0">
                  {initials}
                </div>
              )}
...
// ─── Internal Notes (compact inline version for right panel) ─────────────────
...
function InlineNotesSection({ accountId }: { accountId: number }) {
  ...
  const addNote = useMutation({
    ...
    onSuccess: () => { setContent(""); setAdding(false); qc.invalidateQueries({ queryKey: ["support-notes", accountId] }); },
  });
      ...
      {adding && (
        <div className="mb-2 space-y-1.5">
          ...
          <div className="flex justify-end gap-1.5">
            <button onClick={() => setAdding(false)} className="text-xs text-slate-500 px-2 py-1 rounded hover:bg-slate-50 transition">Cancel</button>
            <button onClick={() => content.trim() && addNote.mutate(content)} disabled={!content.trim() || addNote.isPending}
              className="text-xs bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white px-3 py-1 rounded transition font-medium">
              Save
            </button>
          </div>
        </div>
...
export default function ActivitySnapshotPanel({ accountId, store, owner, subscription, staffCount }: Props) {
  return (

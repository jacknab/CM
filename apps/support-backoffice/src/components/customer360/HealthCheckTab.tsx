...
import HealthCheckSegmentCard, { type SegmentResult } from "./HealthCheckSegmentCard";
...
export default function HealthCheckTab({ accountId }: { accountId: number }) {
  ...
  const { data: latestRun, isLoading: loadingLatest } = useQuery<HealthRun>({
    ...
    queryFn: () => api.accounts.healthCheck.latest(accountId),
    staleTime: 0,
    retry: false,
  });
  ...
  const { data: history } = useQuery<HistoryItem[]>({
    ...
    queryFn: () => api.accounts.healthCheck.history(accountId),
    staleTime: 30_000,
    enabled: showHistory,
  });

  // Specific run (when viewing from history)
  const { data: specificRun, isLoading: loadingSpecific } = useQuery<HealthRun>({
    queryKey: ["health-check-run", accountId, activeRunId],
    queryFn: () => api.accounts.healthCheck.get(accountId, activeRunId!),
    enabled: !!activeRunId,
    staleTime: 0,
  });
  ...
  const runAll = useMutation({
    mutationFn: () => api.accounts.healthCheck.run(accountId),
  ...
  const rerunSegment = async (segmentId: string) => {
    if (!activeRun?.id) {
      // No existing run — do a full run first
      await runAll.mutateAsync();
      return;
    }
    const result = await api.accounts.healthCheck.rerunSegment(accountId, activeRun.id, segmentId);
    // Merge result into current view
  ...
  const saveNote = async () => {
    ...
    setSavingNote(true);
    try {
      await api.accounts.healthCheck.updateNotes(accountId, activeRun.id, note);
  ...
  // Sorted segments from the active run
    ...
    <div className="p-6 max-w-4xl space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-bold text-slate-800">Account Health Check</h2>
          {activeRun && (
      ...
      {!isLoading && !activeRun && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm font-medium text-slate-600 mb-1">No health check run yet</p>
          <p className="text-xs text-slate-400 mb-4">Click "Run All" to run a full diagnostic on this account.</p>
          <button
            onClick={() => runAll.mutate()}
            disabled={runAll.isPending}
          ...
          >
            ...
            {runAll.isPending ? "Running diagnostics…" : "Run Health Check"}
          </button>
        </div>
      )}

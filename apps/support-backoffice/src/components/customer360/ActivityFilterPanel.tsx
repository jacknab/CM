import { Filter, X, ExternalLink, RotateCcw, UserX, Clock, DollarSign, Send, MessageSquare, AlertTriangle, FileText, Calendar, Phone, FileText as InvoiceIcon, Ticket } from "lucide-react";
...
export default function ActivityFilterPanel({ filters, onChange, accountId, onQuickAction }: Props) {
    ...
    <div className="w-52 flex-shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-y-auto scrollbar-thin">
      <div className="p-3 border-b border-slate-100">
        <div className="flex items-center justify-between mb-3">
          ...
          {isDirty && (
            <button
              onClick={() => onChange({ range: "7d", category: "all", customFrom: "", customTo: "" })}
        ...
        <div className="mb-2.5">
          ...
          <select
            ...
            className="w-full text-xs border border-slate-200 rounded-lg px-2 py-2 bg-slate-50 focus:outline-none focus:border-indigo-400 focus:bg-white transition"
          >
      ...
      <SideSection title="Quick Links">
        {[
          ...
          { label: "View Invoices",        icon: <InvoiceIcon size={12} />,  action: "invoices" },
          { label: "View AI Calls",        icon: <Phone size={12} />,        action: "ai_calls" },
          { label: "View Appointments",    icon: <Calendar size={12} />,     action: "appointments" },
        ].map(item => (
          <QuickLinkButton key={item.action} label={item.label} icon={item.icon}
...
function QuickLinkButton({ label, icon, onClick }: { label: string; icon: React.ReactNode; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition group"
    >

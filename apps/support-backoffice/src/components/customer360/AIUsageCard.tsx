...
export default function AIUsageCard({ stats, full }: { stats: AccountOverview["stats"]; full?: boolean }) {
  const bookingRate = stats.aiCallsThisMonth > 0
    ? Math.round((stats.aiBookingsThisMonth / stats.aiCallsThisMonth) * 100)
    : 0;
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-700">AI Receptionist</h3>
        </div>
      ...
      <div className="p-4 space-y-3">
        {stats.aiCallsThisMonth === 0 ? (
          <div className="text-center py-4">
            ...
            <p className="text-xs text-slate-400">No AI calls this month</p>
          </div>
        ) : (
          <>
            {/* Main metric */}

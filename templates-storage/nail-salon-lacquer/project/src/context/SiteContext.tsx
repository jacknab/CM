...
function buildHoursRows(entries: HoursEntry[]): LiveHoursRow[] {
  const sorted = [...entries].sort((a, b) => {
    // Reorder so Monday=start: treat Sunday(0) as 7
...
function toPhone(raw: string): string {
  // normalise E.164 or raw to display
...
export function SiteProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<SiteContextValue>(() => {
    if (!data) {
      // Static fallback (used in thumbnail builder + when no slug is present)
      return {
    ...
    const phoneHref = phoneDigits.length >= 10 ? `tel:+${phoneDigits.startsWith('1') ? phoneDigits : '1' + phoneDigits}` : staticSalon.phoneHref;
    ...
    const validReviews = data.reviews
      .map((review) => ({
        ...review,
        comment: review.comment ?? review.review_text ?? null,
        created_at: review.created_at ?? review.review_create_time ?? null,
      }))
    ...
    const avgRating =
      reviewCount > 0
        ? Math.round((data.reviews.reduce((s, r) => s + r.rating, 0) / reviewCount) * 10) / 10
        : 5;
    ...
    return {
      ...
      reviewCount: reviewCount > 0 ? reviewCount : 327,
      avgRating,
